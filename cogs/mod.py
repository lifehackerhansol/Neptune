...
from utils.utils import is_staff, send_dm_message
...
class Mod(commands.Cog):
    ...
    async def warn(self, ctx, member: Union[discord.Member, discord.User], *, reason=""):
        ...
        warn_count = len(await sql.get_warns(member.id, ctx.guild.id))
        if warn_count >= 5:
            await ctx.send("A user can't have more than 5 warns!")
            return
        await sql.add_warn(member.id, ctx.author.id, ctx.guild.id, reason)
        warn_count += 1
        if isinstance(member, discord.Member):
    ...
    async def delwarn(self, ctx, member: Union[discord.Member, discord.User], idx: int):
        ...
        warns = await sql.get_warns(member.id, ctx.guild.id)
        if not warns:
        ...
        await sql.remove_warn(member.id, ctx.guild.id, idx)
